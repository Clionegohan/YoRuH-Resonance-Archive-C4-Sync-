...
- [x] **THE SYSTEM SHALL** DiffExtractorクラスを提供すること

- [x] **THE SYSTEM SHALL** extract_diff()メソッドで差分テキストを抽出すること
...
- [x] **THE SYSTEM SHALL** vectorize_diff()メソッドで差分テキストをベクトル化すること
  - OllamaClientを使用（mxbai-embed-large）
  - 1024次元ベクトルを返す
  - リトライロジック実装（最大3回、exponential backoff）

- [x] **THE SYSTEM SHALL** 差分テキストが空の場合、vectorize_diff()はNoneを返すこと

- [x] **THE SYSTEM SHALL** ベクトル化に失敗した場合、エラーログを出力しNoneを返すこと
